...
from pvcam.driver import util, version, model
from pvcam.driver.model import HwError
...
class PVCamDLL(CDLL):
    ...
    def __init__(self):
        if os.name == "nt":
            ...
            WinDLL.__init__(self, "pvcam32")

            '''try:
                self.pl_pvcam_init()
                print("Esta Função é Chamada\n")
            except Exception as e:
                print("Esta Função Não é Chamada: " + str(e) + "\n")
                # A Função Printada Abaixo Funciona:
                print(self.pl_error_code())'''
...
class PVCam(model.DigitalCamera):
    ...
    * InGaAs camera cannot do hardware binning
    ...
    def __init__(self, name, role, device, **kwargs):
        ...
        # odemis + drivers
        self._swVersion = "%s (%s)" % (version._get_version(), self.getSwVersion())
...
class PVCamDataFlow(model.df):
    ...
    def synchronized_on(self, event):
        """
